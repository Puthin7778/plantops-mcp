...
if (args.length < 1) {
  console.error(
    `Usage: node ${process.argv[1]} <PLANTOPS_GRAPHQL_ENDPOINT> [ADMIN_SECRET]`
  );
  process.exit(1);
}
const PLANTOPS_ENDPOINT = args[0];
const ADMIN_SECRET = args[1];

console.error(`[INFO] Targeting Plantops Endpoint: ${PLANTOPS_ENDPOINT}`);
if (ADMIN_SECRET) {
  console.error("[INFO] Using Admin Secret.");
} else {
  console.error(
    "[WARN] No Admin Secret provided. Ensure permissions are configured for the default role."
  );
}

const headers: Record<string, string> = {};
if (ADMIN_SECRET) {
  headers["x-plantops-admin-secret"] = ADMIN_SECRET; // Ensure your server accepts this header
}
  ...
  async ({ tableName, limit }) => {
    ...
    try {
      ...
      if (scalarFields.length === 0) {
        console.error(
          `[WARN] No scalar fields found for table ${tableName}...`
        );
...
server.tool(
  ...
  "Performs a simple aggregation (count, sum, avg, min, max)...",
  {
    ...
    filter: z
      .record(z.unknown())
      .optional()
      .describe("Optional. A Plantops GraphQL 'where' filter object..."),
  },
  ...
  async ({ healthEndpointUrl }) => {
    ...
    try {
      ...
      if (healthEndpointUrl) {
        ...
        console.error(
          `[DEBUG] Performing GraphQL query { __typename } to: ${PLANTOPS_ENDPOINT}`
        );
        const query = gql`
          query HealthCheck {
            __typename
        ...
        `;
        const result = await makeGqlRequest(query);
        resultText = `GraphQL endpoint ${PLANTOPS_ENDPOINT} is responsive. Result: ${JSON.stringify(
          result
        )}`;
      }
      return {
...
async function main() {
  console.error(`[INFO] Starting ${SERVER_NAME} v${SERVER_VERSION}...`);
  try {
    await getIntrospectionSchema();
  } catch (error) {
    console.error(`[WARN] Initial schema fetch failed...: ${error}`);
  }

  const transport = new StdioServerTransport();
  console.error("[INFO] Connecting server to STDIO transport...");
